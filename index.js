...
io.on('connection', function(socket) {
  ...
  socket.on('weapon', function(data) {
    ...
    if (
      data.type == 'landmine' ||
      data.type == 'healthPacket' ||
      data.type == 'gunner' ||
      data.type == 'bridge' ||
      data.type == 'healthBeacon'
    ) {
      weapons.push(data);
    }
    if (
      data.type == 'healthPacketRemove' ||
      data.type == 'landmineRemove' ||
      data.type == 'gunnerRemove' ||
      data.type == 'bridgeRemove' ||
      data.type == 'coinRemove' ||
      data.type == 'healthBeaconRemove'
    ) {
  ...
  socket.on('death', function(deathData) {
    ...
    if (!foundKillerMatch) {
      if (
        deathData.killerName.charAt(deathData.killerName.length - 1) != '-' &&
        deathData.killerName.charAt(1) != '-'
      ) {
        scores.push({
    ...
    if (!foundVictimMatch) {
      if (
        deathData.victimName.charAt(deathData.victimName.length - 1) != '-' &&
        deathData.victimName.charAt(1) != '-'
      ) {
        scores.push({
...
function saveAmmo(data) {
  for (var i = 0; i < ammo.length; i++) {
    if (data.name == ammo[i].name) {
      ...
      ammo[i].healthBeacon = data.healthBeacon;
...
function loadJSON(filename) {
  var object = JSON.parse(
    fs.readFileSync('./data/' + filename + '.json', 'utf8')
  );
  return object;
}
